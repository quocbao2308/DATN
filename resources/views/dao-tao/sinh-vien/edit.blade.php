...
@section('content')
    <div class="page-heading">
        ...
        <section class="section">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('dao-tao.sinh-vien.update', $sinhVien->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    ...
                                    <input type="date" name="ngay_sinh" class="form-control @error('ngay_sinh') is-invalid @enderror" 
                                        value="{{ old('ngay_sinh', $sinhVien->ngay_sinh?->format('Y-m-d')) }}">
                                    @error('ngay_sinh')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="nganh_id" class="form-select @error('nganh_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($nganhs as $nganh)
                                            <option value="{{ $nganh->id }}" {{ old('nganh_id', $sinhVien->nganh_id) == $nganh->id ? 'selected' : '' }}>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="chuyen_nganh_id" class="form-select @error('chuyen_nganh_id') is-invalid @enderror">
                                        ...
                                        @foreach($chuyenNganhs as $cn)
                                            <option value="{{ $cn->id }}" {{ old('chuyen_nganh_id', $sinhVien->chuyen_nganh_id) == $cn->id ? 'selected' : '' }}>
                                                {{ $cn->ten_chuyen_nganh }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="khoa_hoc_id" class="form-select @error('khoa_hoc_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($khoaHocs as $kh)
                                            <option value="{{ $kh->id }}" {{ old('khoa_hoc_id', $sinhVien->khoa_hoc_id) == $kh->id ? 'selected' : '' }}>
                                                {{ $kh->ten_khoa_hoc }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="trang_thai_hoc_tap_id" class="form-select @error('trang_thai_hoc_tap_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($trangThais as $tt)
                                            <option value="{{ $tt->id }}" {{ old('trang_thai_hoc_tap_id', $sinhVien->trang_thai_hoc_tap_id) == $tt->id ? 'selected' : '' }}>
                                                {{ $tt->ten_trang_thai }}
                                            </option>
                                        @endforeach
                                    </select>
