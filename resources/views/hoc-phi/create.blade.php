        ...
        <div class="mb-4">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Tạo phiếu thu</li>
                </ol>
            </nav>
            <h2>Tạo phiếu thu học phí</h2>
        </div>

        <!-- Form Card -->
        <div class="card">
            <div class="card-body">
                <form action="{{ route('dao-tao.hoc-phi.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row">
                            ...
                            <select name="sinh_vien_id" id="sinh_vien_id"
                                ...
                                @foreach ($danhSachSinhVien as $sinhVien)
                                    <option value="{{ $sinhVien->id }}"
                                        {{ old('sinh_vien_id') == $sinhVien->id ? 'selected' : '' }}>
                            ...
                            <select name="hoc_ky_id" id="hoc_ky_id"
                                ...
                                @foreach ($danhSachHocKy as $hocKy)
                                    <option value="{{ $hocKy->id }}"
                                        {{ old('hoc_ky_id') == $hocKy->id ? 'selected' : '' }}>
                                        {{ $hocKy->ten_hoc_ky }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="trang_thai" id="trang_thai"
                                ...
                                <option value="chua_nop" {{ old('trang_thai') == 'chua_nop' ? 'selected' : '' }}>Chưa nộp
                                </option>
                                <option value="da_nop" {{ old('trang_thai') == 'da_nop' ? 'selected' : '' }}>Đã nộp
                                </option>
                                <option value="no" {{ old('trang_thai') == 'no' ? 'selected' : '' }}>Nợ</option>
                            </select>
                            @error('trang_thai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="ngay_nop" id="ngay_nop"
                                class="form-control @error('ngay_nop') is-invalid @enderror" value="{{ old('ngay_nop') }}">
                            @error('ngay_nop')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="ghi_chu" id="ghi_chu" rows="3" class="form-control @error('ghi_chu') is-invalid @enderror">{{ old('ghi_chu') }}</textarea>
                            @error('ghi_chu')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save me-1"></i> Lưu phiếu thu
                        </button>
